...
import {
  ...
  import {configEdu, getTotalStepsForAllLevelsEdu} from '$lib/components/SideBar/configEdu'

  let activeItemSlug: string = ''
  let userProgress: number = 0
  let totalProgress: number = getTotalStepsForAllLevelsEdu()
...
{#each Object.entries(configEdu) as [level, levelDetails]}
